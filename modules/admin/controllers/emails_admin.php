 ...
 *  *  
 */
class Emails_Admin extends Admin_Controller {

	public function index($page = 1){
		$items = ORM::factory("page")->where(array("type"=>"email"))->limit($this->config["per_page"])->offset(($page-1)*$this->config["per_page"])->find_all();
		$view  = new View("emails/index");
		$view->items = $items;
		$actions = new View("emails/actions");
	...
	public function update(){
		...
		$item->save();
		url::redirect(url::base()."admin/emails?success");
	}
