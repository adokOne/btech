...
class User_Admin extends Admin_Controller {
	...
	public function new_one(){
		...
		$view->roles = ORM::factory("role")->where("name","wholesale")->find_al();
		$this->view->content = $view->render(false);
		$this->view->render(true);
	}
	...
	public function edit($id=false){
		...
		$view = new View("user/edit");
		$view->user = $user;
		$view->roles = ORM::factory("role")->where("name","wholesale")->find_all();
		$this->view->content = $view->render(false);
		$this->view->render(true);
	...
	public function update(){
		...
		$user->save();
		foreach(ORM::factory('role')->where("name","wholesale")->find_all() as $item){
			$user->remove($item);
		}
		...
		$user->save();
		url::redirect(url::base()."/admin/user?success");
	}
