        	...
        	<div class="wrapper">
                ...
                <ul id="nav">
                    <?php foreach($lang["top_menu"] as $link=>$text):?>
                        <li class="<?php echo url::current()=="main" && empty($link) || !empty($link) && strpos(url::current(),$link) !== false ? "active ".(!empty($link) ? $link : 'index') :  $link   ?>" ><a href="<?php echo url::lang_url().$link?>"><?php echo $text; ?></a></li>
                    <?php endforeach;?>
                </ul>
                <!-- end nav -->
                <div class="clear"></div>
